...
(( strcmp(contacts->contact(i).collision1().c_str(), COLLISION_ARM) == 0 ) ||
...
#endif
		{
			rewardHistory = (maxEpisodeLength-episodeFrames)*REWARD_WIN;

			//rewardHistory = maxEpisodeLength * REWARD_WIN;
			newReward  = true;
			endEpisode = true;
...
bool ArmPlugin::updateAgent()
{
	...
	velocity = vel[action/2];
	if (action%2 == 0) // even action
...
void ArmPlugin::OnUpdate(const common::UpdateInfo& updateInfo)
{
	...
	if( maxEpisodeLength > 0 && episodeFrames > maxEpisodeLength )
	{
		...
		rewardHistory = maxEpisodeLength * REWARD_LOSS;
		newReward     = true;
		endEpisode    = true;
	}
	...
	if( hadNewState && !newReward )
	{
		...
		physics::LinkPtr base  = model->GetLink(BASE_NAME);
		...
		const math::Box& baseBBox = base->GetBoundingBox();
		...
		if(gripBBox.min.z <= baseBBox.min.z)
		{
						
			printf("Gripper hit the ground.\n");
		...
		else if(gripBBox.min.z > baseBBox.min.z)
		{
			...
			{				
				...
				avgGoalDelta = (avgGoalDelta * ALPHA) + (lastGoalDistance - distGoal) * (1 - ALPHA);
				
				
		
				
				rewardHistory =  avgGoalDelta * REWARD_WIN * exp(-distGoal);
				//rewardHistory = avgGoalDelta * REWARD_WIN;
				//rewardHistory = REWARD_WIN * exp(-distGoal);
